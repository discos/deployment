...
public class DALImpl extends JDALPOA implements Recoverer {
	...
	private ThreadLocal<SAXParser> saxParserLocal;
...
//		profilingThreadLoopRunner.runLoop();
	...
	public void loadFactory() {
	    ...
	    try {
			...
			final String allURIs = getSchemas();
			if (allURIs == null) {
				m_logger.log(AcsLogLevel.NOTICE,"Schema files: NO SCHEMAS!");
			}

			saxParserLocal = new ThreadLocal<SAXParser>() {
				@Override
				protected SAXParser initialValue()
				{
					try {
						SAXParser parser = factory.newSAXParser();
			            m_logger.log(AcsLogLevel.DELOUSE, "created parser " + parser.getClass().getName() +
					            " with schema location: '" + allURIs +"'");//msc:added
						if (allURIs != null) {
							parser.setProperty(EXTERNAL_SCHEMA_LOCATION_PROPERTY_ID, allURIs);
						}
						...
						parser.setProperty("http://apache.org/xml/properties/input-buffer-size", new Integer(1024 * 10));
                        ...
                        //parser.setProperty("http://apache.org/xml/features/validation/schema/augment-psvi", true);
						return parser;
					} catch (Exception e) {
						m_logger.log(AcsLogLevel.NOTICE, "Cannot instantiate local SAXParser", e);
						return null;
					}
				}
			};
	...
	public void parseNode(DALNode node, XMLHandler xmlSolver, String path) throws SAXException, IOException, AcsJCDBXMLErrorEx {
		...
		if(curlNode != null) {
			...
			if (xml != null)
			{
                saxParserLocal.get().parse(new InputSource(new StringReader(xml)), xmlSolver);
			}
			else
			{
				xmlFile = new File(xmlPath);
				xmlSolver.setFirstElement(node.name);
				saxParserLocal.get().parse(xmlFile, xmlSolver);
			}
